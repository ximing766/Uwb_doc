...
本文档详细描述了UWB与Reader之间的通信协议规范。crewai mcp test 12
...
> 1. 无特殊要求，C1/C3 可忽略  
> 2. HALT为固定C2命令,如示例4.5.
> 3. 发送时间戳为 4 字节 16 进制，小端存储。
> 4. C5指令中Uwb返回信息按照apd解析,当前版本仅1条软件版本
> 5. 指令参考:
```c
//4.1 UWB -> Reader 时间戳请求
00 00 FF 11 00 06 FF FF FF FF FF 05 FF FF FF FF FF 01 C4 00 01 00 39 00
